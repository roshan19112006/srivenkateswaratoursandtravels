...
/**
 * Compact Individual Service Card with counter-parallax inner image and depth layers
 */
function ParallaxServiceCard({
  ...
  // Parallax shifts:
  // 1. Image shifts horizontally within its framed window in opposite direction
  ...
  // 2. Watermark number shifts with subtle parallax
      ...
      <div className="p-4 sm:p-5 relative z-10">
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-brand-500/15 border border-brand-400/30 text-brand-300 text-[11px] font-bold uppercase tracking-wider">
            ...
            <span>{service.badge || 'Sri Venkateswara'}</span>
          </div>
        ...
        {/* Framed Parallax Window Image (compact height) */}
        <div className="relative h-32 sm:h-36 w-full rounded-xl overflow-hidden bg-slate-950 mb-3.5 border border-slate-800/60 shadow-inner">
          <motion.img
            src={service.image}
            alt={service.title}
          ...
          {/* Vehicle recommendation pill inside image corner */}
          {service.vehicle && (
            <div className="absolute bottom-2 left-2 bg-slate-950/85 backdrop-blur-md border border-slate-700/60 px-2 py-0.5 rounded-md text-[10px] font-semibold text-slate-200 flex items-center gap-1 shadow-sm">
              <Car className="w-2.5 h-2.5 text-gold-400" />
        ...
        {/* Highlights tags - compact 2 items */}
        {service.highlights && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {service.highlights.slice(0, 2).map((tag, tIdx) => (
              <span
                key={tIdx}
...
export default function Services({ onSelectService }) {
  ...
  // Map progress to pixel translation for pixel-accurate alignment across all displays
  const trackX = useTransform(smoothProgress, [0, 1], [0, -maxScroll]);

  // Background Parallax Kinetic Typography Layer
  const bgTextX = useTransform(smoothProgress, [0, 1], ['4%', '-35%']);
  ...
  useEffect(() => {
    const unsubscribe = smoothProgress.on('change', (latest) => {
      ...
      const idx = Math.min(
        ...
        Math.floor(p * servicesData.length + 0.3)
      );
      setActiveIndex(idx);
    });
  ...
  const scrollToCard = (index) => {
    ...
    const targetY = sectionTop + (index / (servicesData.length - 1)) * totalScrollableHeight;
  ...
  const handleNext = () => {
    scrollToCard(Math.min(servicesData.length - 1, activeIndex + 1));
  };
  ...
  return (
    <section
      id="services"
      ref={sectionRef}
      className="relative h-[200vh] sm:h-[230vh] bg-slate-950 text-white selection:bg-brand-500 selection:text-white"
    >
      ...
      <div className="sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-between pt-20 sm:pt-24 pb-4 sm:pb-6">
        ...
        >
          <span className="text-[10vw] font-black uppercase tracking-tight text-white/5 font-display stroke-text">
            TRAVEL MADE SIMPLE • CHENNAI COMMUTE • AIRPORT TRANSFERS • OUTSTATION ROADS • EXECUTIVE FLEET •
          </span>
        </motion.div>
        ...
        <div className="relative z-20 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs sm:text-sm text-slate-300 max-w-lg font-normal">
                Glide through our tailored travel categories. Comfortable, dependable rides for every journey.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 self-start md:self-end">
              ...
              <div className="hidden sm:flex flex-col items-end">
                <span className="text-[11px] font-mono font-bold text-slate-400">
                  CATEGORY {String(activeIndex + 1).padStart(2, '0')} OF {String(servicesData.length).padStart(2, '0')}
                </span>
          ...
          <div className="flex items-center justify-between gap-4 text-xs text-slate-400 border-t border-slate-800/80 pt-2.5">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="font-semibold text-slate-300 text-[11px] hidden sm:inline">
                Scroll down or use arrows to glide across categories
              </span>
            ...
            {/* Visual Progress Line & Category Dots */}
            <div className="flex items-center gap-1.5">
              {servicesData.map((service, idx) => (
                <button
                  key={service.id}
                  ...
                  className={`transition-all duration-300 rounded-full cursor-pointer ${
                    activeIndex === idx
                      ? 'w-6 sm:w-8 h-1.5 bg-brand-400 shadow-[0_0_8px_rgba(96,165,250,0.8)]'
            ...
            <div className="text-right text-[11px]">
              ...
              <span className="hidden sm:inline text-brand-400 font-bold ml-1">★ 24/7 Fleet</span>
            </div>
